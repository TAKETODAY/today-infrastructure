 ...
 * for the <em>interrupting</em> path ({@code mayInterruptIfRunning == true}).
 * <p>
 * This path transitions {@code NEW -> COMPLETING -> INTERRUPTING -> INTERRUPTED},
 * introducing an extra intermediate state. Even when a reader observes
 * {@code INTERRUPTING} or {@code INTERRUPTED} via {@link Future#isCancelled()},
 * {@link Future#getCause()} must never be {@code null} — the cancellation cause is
 * written into the non-volatile {@code result} field before the interrupt state is
 * published.
 *
 ...
 */
@JCStressTest
@Description("中断取消后 getCause() 必须非 null（无可见性窗口）")
// 两次读都观察到尚未取消 —— 合法
...
@State
public class FutureCancelInterruptVisibilityTests {

  final Promise<Object> future = Future.forPromise();

  @Actor
  public void cancel() {
    future.cancel("test reason", true);
  }
