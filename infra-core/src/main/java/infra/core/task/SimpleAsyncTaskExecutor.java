...
public class SimpleAsyncTaskExecutor extends CustomizableThreadCreator implements AsyncTaskExecutor, Serializable, AutoCloseable {
   ...
   * @see #close()
   * @see infra.scheduling.concurrent.ExecutorConfigurationSupport#setAwaitTerminationMillis
   */
   ...
   *
   * @see infra.scheduling.concurrent.ThreadPoolTaskExecutor#setMaxPoolSize
   */
  ...
  public void execute(Runnable task, long startTimeout) {
    ...
    var future = task instanceof java.util.concurrent.Future<?> f ? f : null;
    if (isThrottleActive() && startTimeout > TIMEOUT_IMMEDIATE) {
      this.concurrencyThrottle.beforeAccess();
      try {
        doExecute(new TaskTrackingRunnable(task, future));
      }
      catch (Throwable ex) {
  ...
  private void checkCancelled(java.util.concurrent.@Nullable Future<?> future) {
  ...
  private final class TaskTrackingRunnable implements Runnable {
    ...
    private final java.util.concurrent.@Nullable Future<?> future;

    public TaskTrackingRunnable(Runnable task, java.util.concurrent.@Nullable Future<?> future) {
