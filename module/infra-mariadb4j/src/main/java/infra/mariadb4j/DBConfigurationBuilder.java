...
import static infra.mariadb4j.DBConfiguration.Executable.Dump;
import static infra.mariadb4j.DBConfiguration.Executable.InstallDB;
import static infra.mariadb4j.DBConfiguration.Executable.PrintDefaults;
import static infra.mariadb4j.DBConfiguration.Executable.Server;
...
public class DBConfigurationBuilder {
  ...
  protected File baseDir = ApplicationTemp.instance.getDir("MariaDB4j/base").toFile();

  protected File libDir;

  protected File dataDir = ApplicationTemp.instance.getDir(DEFAULT_DATA_DIR).toFile();

  protected File tmpDir = ApplicationTemp.instance.getDir(DEFAULT_TMP_DIR).toFile();

  protected String socket; // see _getSocket()
  ...
  public DBConfiguration build() {
    if (dataDir == null || tmpDir == null) {
      String p = ApplicationTemp.instance + "/" + path();
  ...
  protected File _getDataDir() {
    if (getDataDir() == null || getDataDir().equals(ApplicationTemp.instance.getDir(DEFAULT_DATA_DIR).toFile())) {
      return new File(ApplicationTemp.instance + File.separator + DEFAULT_DATA_DIR + File.separator + _getPort());
    }
    return getDataDir();
  }

  protected File _getTmpDir() {
    if (getTmpDir() == null || getTmpDir().equals(ApplicationTemp.instance.getDir(DEFAULT_TMP_DIR).toFile())) {
      return new File(ApplicationTemp.instance
              + File.separator
              + DEFAULT_TMP_DIR
              + File.separator
              + getPort());
  ...
  protected String getBinariesClassPathLocation() {
    return "ch/vorburger/mariadb4j/" + _getDatabaseVersion() + "/" + getOS();
  }
  ...
  protected String _getBinariesClassPathLocation() {
    ...
    return null; // see DB.unpackEmbeddedDb()
  }
  ...
  public DBConfigurationBuilder setExecutable(Executable executable, String path) {
    ...
    executables.put(
            ...
            () -> new File(Objects.requireNonNull(path, "path")));
    return this;
  }
  ...
  public DBConfigurationBuilder setExecutable(
    ...
    executables.put(
            Objects.requireNonNull(executable, "executable"),
            Objects.requireNonNull(pathSupplier, "pathSupplier"));
    return this;
  }
