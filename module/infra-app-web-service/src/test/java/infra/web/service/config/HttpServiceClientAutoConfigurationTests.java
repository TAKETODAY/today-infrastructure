...
import infra.web.service.registry.HttpServiceGroupConfigurer.ClientCallback;
import infra.web.service.registry.HttpServiceGroupConfigurer.Groups;
  ...
  void configuresClientFromPropertiesWhenHasHttpClientAutoConfiguration() {
    this.contextRunner.withConfiguration(AutoConfigurations.of(HttpClientAutoConfiguration.class))
            .withPropertyValues("http.service-client.one.connect-timeout=5s",
                    "http.service-client.two.connect-timeout=10s")
            .withUserConfiguration(HttpClientConfiguration.class, MockRestServiceServerConfiguration.class)
            .run((context) -> {
