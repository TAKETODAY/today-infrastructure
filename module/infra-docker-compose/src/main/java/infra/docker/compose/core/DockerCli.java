...
import infra.docker.compose.core.DockerCliCommand.ComposeVersion;
import infra.docker.compose.core.DockerCliCommand.Type;
...
class DockerCli {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DockerCli.class);
  ...
  private static class DockerCommands {
    ...
    private DockerCommand getDockerCommand(ProcessRunner processRunner) {
      try {
        ...
        logger.trace(LogMessage.format("Using docker {}", version));
    ...
    private DockerCommand getDockerComposeCommand(ProcessRunner processRunner) {
      try {
        ...
        logger.trace(LogMessage.format("Using Docker Compose {}", response.version()));
