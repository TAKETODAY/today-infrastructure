...
/**
 * Tests for {@link infra.web.server.error.ErrorAttributeOptions}.
 *
...
class ErrorAttributesOptionsTests {
  ...
  void includingFromEmptyAttributesReturnAddedEntry() {
    infra.web.server.error.ErrorAttributeOptions options = of(EnumSet.noneOf(Include.class));
  ...
  void includingFromMatchingAttributesDoesNotModifyOptions() {
    infra.web.server.error.ErrorAttributeOptions options = of(EnumSet.of(Include.EXCEPTION, Include.STACK_TRACE));
    assertThat(options.including(Include.EXCEPTION).getIncludes()).containsOnly(Include.EXCEPTION,
            Include.STACK_TRACE);
  }
