...
public class ConnectionDetailsFactories {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ConnectionDetailsFactories.class);
  ...
  record Registration<S, D extends ConnectionDetails>(
          Class<S> sourceType, Class<D> connectionDetailsType, ConnectionDetailsFactory<S, D> factory) {

    @SuppressWarnings("unchecked")
    private static <S, D extends ConnectionDetails> @Nullable Registration<S, D> get(ConnectionDetailsFactory<S, D> factory) {
