...
Web MVC 和 Reactive 对相同的底层方法验证具有内置支持，但不需要 AOP。因此，请务必查看本节的其余部分，并参阅 Web MVC xref:web/webmvc/mvc-controller/ann-validation.adoc[验证] 和 xref:web/webmvc/mvc-ann-rest-exceptions.adoc[错误响应] 部分。
