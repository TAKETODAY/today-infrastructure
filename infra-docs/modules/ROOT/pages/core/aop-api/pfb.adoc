...
`infra.aop.framework.ProxyFactoryBean`. This gives complete control over
...
The `ProxyFactoryBean`, like other Infra `FactoryBean` implementations, introduces a
...
In common with most `FactoryBean` implementations provided with Infra-, the
...
CGLIB proxying works by generating a subclass of the target class at runtime. Infra
...
in the `infra-core` JAR. In other words, CGLIB-based AOP works "out of the box", as do
JDK dynamic proxies.
