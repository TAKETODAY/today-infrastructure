...
对于集成测试特定的 AOT 支持, 参见 xref:testing/testcontext-framework/aot.adoc[测试的 AOT 支持].
...
使用常规运行时启动此应用程序会经历多个步骤，包括类路径扫描、配置类解析、Bean 实例化和生命周期回调处理。
AOT 处理的刷新只涵盖了 xref:core/beans/introduction.adoc[常规 `refresh`] 中发生的一部分内容。
可以通过以下方式触发 AOT 处理：
...
为了更有针对性地发现和测试，Infra Framework 提供了一个专用模块，核心 AOT 测试工具 `"cn.taketoday:infra-core-test"`。
...
INFO: Infra version:5.0.0-SNAPSHOT
