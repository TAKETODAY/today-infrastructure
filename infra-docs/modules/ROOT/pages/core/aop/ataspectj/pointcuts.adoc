...
when advice runs. Infra AOP only supports method execution join points for Spring
...
* Any join point (method execution only in Infra AOP) on a Spring bean named
`tradeService`:
+
[literal,indent=0,subs="verbatim"]
----
...
* Any join point (method execution only in Infra AOP) on Spring beans having names that
