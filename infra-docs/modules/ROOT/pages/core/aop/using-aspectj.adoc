...
distribution as `infra-aspects.jar`. You need to add this to your classpath in order
...
`infra-aspects.jar` contains an annotation-driven aspect that exploits this
...
`AnnotationBeanConfigurerAspect` in `infra-aspects.jar` that acts on the presence of
...
In addition to the `@Configurable` aspect, `infra-aspects.jar` contains an AspectJ
...
TIP: `infra-aspects` provides a similar aspect that offers the
...
management support but do not want to (or cannot) use annotations, `infra-aspects.jar`
...
`infra-aspects.jar` to configure the aspect instances once they have bean created by
the AspectJ runtime.
...
later in this section) `-javaagent:path/to/infra-instrument.jar`. Developers configure
...
`infra-instrument.jar` that was supplied as the value of the `-javaagent` argument in
...
Infra-specific (but very general) JVM agent, `infra-instrument.jar`, autodetected
