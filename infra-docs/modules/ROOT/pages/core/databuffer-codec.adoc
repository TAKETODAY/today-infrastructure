...
`infra-core` 模块提供了一组抽象，以处理各种字节缓冲区 API，如下所示：
...
`infra-core` 模块提供了 `byte[]`、`ByteBuffer`、`DataBuffer`、`Resource` 和 `String`
的编码器和解码器实现。`infra-web` 模块添加了 Jackson JSON、Jackson Smile、JAXB2、Protocol Buffers 和其他编码器和解码器。
