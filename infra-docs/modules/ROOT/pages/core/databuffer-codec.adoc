...
The `today-core` module provides a set of abstractions to work with various byte buffer
APIs as follows:
...
The `cn.taketoday.core.codec` package provides the following strategy interfaces:
...
The `today-core` module provides `byte[]`, `ByteBuffer`, `DataBuffer`, `Resource`, and
`String` encoder and decoder implementations. The `spring-web` module adds Jackson JSON,
Jackson Smile, JAXB2, Protocol Buffers and other encoders and decoders.
