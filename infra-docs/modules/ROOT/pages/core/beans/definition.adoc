...
A Infra IoC container manages one or more beans. These beans are created with the
...
the `StandardBeanFactory` implementation. `StandardBeanFactory` supports
...
Also, if you use Infra AOP, it helps a lot when applying advice to a set of beans
related by name.
****

NOTE: With component scanning in the classpath, Infra generates bean names for unnamed
...
defined by `java.beans.Introspector.decapitalize` (which Infra uses here).
...
compatible with Infra. That is, the class being developed does not need to implement
...
The Infra IoC container can manage virtually any class you want it to manage. It is
not limited to managing true JavaBeans. Most Infra users prefer actual JavaBeans with
...
that absolutely does not adhere to the JavaBean specification, Infra can manage it as
well.
...
NOTE: In Infra documentation, "factory bean" refers to a bean that is configured in the
Infra container and that creates objects through an
...
`FactoryBean` (notice the capitalization) refers to a Infra-specific
