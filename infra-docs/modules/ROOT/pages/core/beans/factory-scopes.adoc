...
`request`、`session` 作用域仅在您使用感知 Web 的 Infra `ApplicationContext` 实现时才可用。
...
作为工厂作用域的替代方案，Infra `WebApplicationContext` 还支持将 `HttpMockRequest`、`HttpMockResponse`、`Session`、`WebRequest` 以及（如果存在 JSF）`FacesContext` 和 `ExternalContext` 注入到 Infra 管理的 bean 中，只需通过基于类型的自动装配，就像其他 bean 的常规注入点一样。
