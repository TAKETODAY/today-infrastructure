...
`cn.taketoday.web.context.request.RequestContextListener` `ServletRequestListener`.
...
`ServletContext` attribute. This is somewhat similar to a Infra singleton bean but
differs in two important ways: It is a singleton per `ServletContext`, not per Infra
