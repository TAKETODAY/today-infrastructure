...
有关 RAR 部署所涉及的配置详细信息，请参阅
{today-framework-api}/jca/context/SpringContextResourceAdapter.html[`SpringContextResourceAdapter`] 类的 javadoc。
...
. 添加一个 `META-INF/ra.xml` 部署描述符（如 {today-framework-api}/jca/context/SpringContextResourceAdapter.html[`SpringContextResourceAdapter` 的 javadoc] 中所示）
和相应的 Infra XML bean 定义文件（通常是 `META-INF/applicationContext.xml`）。
