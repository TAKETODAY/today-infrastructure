...
therefore, provides internationalization ("`i18n`") functionality. Infra also provides the
`HierarchicalMessageSource` interface, which can resolve messages hierarchically.
Together, these interfaces provide the foundation upon which Infra effects message
...
Infra provides three `MessageSource` implementations, `ResourceBundleMessageSource`, `ReloadableResourceBundleMessageSource`
...
NOTE: As an alternative to `ResourceBundleMessageSource`, Infra provides a
...
files from any Infra resource location (not only from the classpath) and supports hot
...
TIP: As of Infra 4.2, the event infrastructure has been significantly improved and offers
...
The following table describes the standard events that Infra provides:
...
`ApplicationEventPublisherAware` and registering it as a Infra bean. The following
...
At configuration time, the Infra container detects that `EmailService` implements
...
`ApplicationListener` and register it as a Infra bean. The following example
shows such a class:
...
NOTE: Infra eventing mechanism is designed for simple communication between Infra beans
...
architectures that build upon the well-known Infra programming model.
...
The `ApplicationContext` manages the lifecycle of Infra applications and provides a rich
...
This namespace is reserved for internal Infra usage and is subject to change.
