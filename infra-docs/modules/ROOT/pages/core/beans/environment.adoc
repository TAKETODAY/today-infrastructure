...
current environment. Over time, Infra users have devised a number of ways to
...
JNDI lookups, by using either Infra `JndiTemplate`/`JndiLocatorDelegate` helpers or the
...
another. Resolution between all factory methods for the same bean follows Infra
...
Now that we have updated our configuration, we still need to instruct Infra which
...
the Infra bean named `dataSource`.
...
profiles can be declared by using the `@ActiveProfiles` annotation in the `today-test`
...
Infra `Environment` abstraction provides search operations over a configurable
...
In the preceding snippet, we see a high-level way of asking Infra whether the `my-property` property is
...
Infra {today-framework-api}/core/env/StandardEnvironment.html[`StandardEnvironment`]
