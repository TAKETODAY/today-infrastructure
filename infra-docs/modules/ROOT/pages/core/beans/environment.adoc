...
JNDI、servlet 上下文参数、临时 `Properties` 对象、`Map` 对象等等。`Environment`
...
JVM 系统属性、`web.xml` 中的 servlet 上下文参数，甚至是 JNDI 条目指定
...
会填充额外的默认属性源，包括 servlet 配置、servlet 上下文参数，以及如果 JNDI 可用，则会添加一个
...
对于通用的 `StandardServletEnvironment`，完整的层次结构如下，优先级最高的条目位于顶部：
