...
JNDI 上下文参数、临时 `Properties` 对象、`Map` 对象等等。`Environment`
...
JVM 系统属性 上下文参数，甚至是 JNDI 条目指定
（参见 xref:core/beans/environment.adoc#beans-property-source-abstraction[`PropertySource` 接口]）。
在集成测试中，可以通过在 `infra-test` 模块中使用 `@ActiveProfiles` 注解来声明活动的 profiles
...
- ServletConfig 参数（如果适用 -- 例如，在 `MockDispatcherHandler` 上下文中）
