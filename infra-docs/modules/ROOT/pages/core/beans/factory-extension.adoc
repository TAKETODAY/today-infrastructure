...
{today-framework-api}/beans/factory/config/BeanPostProcessor.html[`BeanPostProcessor`]
and {today-framework-api}/core/Ordered.html[`Ordered`] interfaces. See also the note on
...
the Infra IoC container instantiates a bean instance and then `BeanPostProcessor`
...
The `cn.taketoday.beans.factory.config.BeanPostProcessor` interface consists of
...
class itself or at least the `cn.taketoday.beans.factory.config.BeanPostProcessor`
...
`@Resource` (which may fall back to autowiring), Infra might access unexpected beans
...
Infra accesses other beans for matching them by type.
====
...
<beans xmlns="http://www.springframework.org/schema/beans"
  ...
  <lang:groovy id="messenger"
      script-source="classpath:cn/taketoday/scripting/groovy/Messenger.groovy">
...
----
Bean 'messenger' created : cn.taketoday.scripting.groovy.GroovyMessenger@272961
cn.taketoday.scripting.groovy.GroovyMessenger@272961
----
...
Infra `AutowiredAnnotationBeanPostProcessor` -- a `BeanPostProcessor` implementation
...
`cn.taketoday.beans.factory.config.BeanFactoryPostProcessor`. The semantics of
...
That is, the Infra IoC container lets a `BeanFactoryPostProcessor` read the
...
{today-framework-api}/beans/factory/config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`]
and {today-framework-api}/core/Ordered.html[`Ordered`] interfaces for more details.
...
With the `context` namespace introduced in Infra 2.5, you can configure property placeholders
...
it checks against Infra `Environment` properties and regular Java `System` properties.
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
<bean class="cn.taketoday.beans.factory.config.PropertySourcesPlaceholderConfigurer">
...
With the `context` namespace introduced in Infra 2.5, it is possible to configure
...
You can implement the `cn.taketoday.beans.factory.FactoryBean` interface for objects that
are themselves factories.

The `FactoryBean` interface is a point of pluggability into the Infra IoC container's
...
The `FactoryBean` concept and interface are used in a number of places within the Infra
Framework. More than 50 implementations of the `FactoryBean` interface ship with Infra
itself.
