...
The `infra.beans.factory.config.BeanPostProcessor` interface consists of
...
`infra.beans.factory.config.BeanFactoryPostProcessor`. The semantics of
...
You can implement the `infra.beans.factory.FactoryBean` interface for objects that
