...
| `ServletConfigAware`
| Current `ServletConfig` the container runs in. Valid only in a web-aware Infra
  `ApplicationContext`.
| xref:web/webmvc.adoc#mvc[Web MVC]

| `ServletContextAware`
| Current `ServletContext` the container runs in. Valid only in a web-aware Infra
  `ApplicationContext`.
| xref:web/webmvc.adoc#mvc[Web MVC]
|===
