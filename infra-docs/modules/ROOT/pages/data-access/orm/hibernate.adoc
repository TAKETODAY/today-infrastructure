...
We start with a coverage of https://hibernate.org/[Hibernate 5] in a Infra environment,
using it to demonstrate the approach that Infra takes towards integrating OR mappers.
...
[NOTE]
====
As of TODAY Framework 6.0, Infra requires Hibernate ORM 5.5+ for Infra
...
== `SessionFactory` Setup in a Infra Container
...
Infra container. Application objects that need to access resources receive references
...
<beans>
  ...
  <bean id="mySessionFactory" class="infra.orm.hibernate5.LocalSessionFactoryBean">
...
[NOTE]
====
Infra also provides a `LocalSessionFactoryBuilder` variant, seamlessly integrating
...
Infra bean definition, the DAO would resemble the following:
...
of any Infra class is required. This is appealing from a non-invasiveness
...
`SessionFactory.getCurrentSession()` method for any Infra transaction strategy,
returning the current Infra-managed transactional `Session`, even with
...
able to participate in Infra-managed transactions.
...
transaction interceptor. You can configure this transaction interceptor in a Infra
...
Infra container to find these annotations and provide transactional semantics for
...
<beans xmlns="http://www.springframework.org/schema/beans"
  ...
  <bean id="transactionManager"
      class="infra.orm.hibernate5.HibernateTransactionManager">
...
exist on the implementation of the surrounding business service. It needs only a Infra
...
a transaction manager and a business service definition in a Infra application context
...
<beans>
  ...
  <bean id="myTxManager" class="infra.orm.hibernate5.HibernateTransactionManager">
    <property name="sessionFactory" ref="mySessionFactory"/>
  </bean>
...
matter of the transaction strategy that you use. Compared to a Infra-defined local
...
Infra-driven transactions can work as well with a locally defined Hibernate
...
definitions and Infra-driven transactions.
...
to which it synchronizes (along with Infra). You have two options for doing this:

* Pass your Infra `JtaTransactionManager` bean to your Hibernate setup. The easiest
  ...
  Infra then makes the corresponding JTA strategies available to Hibernate.
...
* Among other activities, this synchronization can trigger a callback by Infra to
...
* Infra `JtaTransactionManager` is synchronized to the JTA transaction, so the
  ...
  transaction manager.
* Infra is aware that Hibernate itself is synchronized to the JTA transaction and
