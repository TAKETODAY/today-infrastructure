...
NOTE: `@EnableTransactionManagement` 和 `<tx:annotation-driven/>` 仅在定义它们的同一个应用程序上下文中查找 `@Transactional` bean。这意味着，如果您将注解驱动的配置放在 `MockDispatcherHandler` 的 `WebApplicationContext` 中，它只会检查控制器中的 `@Transactional` bean，而不会检查服务中的。有关更多信息，请参阅 xref:web/webmvc/mvc-core.adoc[MVC]。
