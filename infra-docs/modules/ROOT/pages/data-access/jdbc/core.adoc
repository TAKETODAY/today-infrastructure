...
NOTE: 默认的异常转换器是 `SQLExceptionSubclassTranslator`，它通过一些额外的检查检测 JDBC 4 `SQLException` 子类，并通过 `SQLStateSQLExceptionTranslator` 回退到 `SQLState` 内省。这通常足以满足常见的数据库访问需求，不需要特定于供应商的检测。为了向后兼容，请考虑如下所述使用 `SQLErrorCodeSQLExceptionTranslator`，可能带有自定义错误代码映射。
