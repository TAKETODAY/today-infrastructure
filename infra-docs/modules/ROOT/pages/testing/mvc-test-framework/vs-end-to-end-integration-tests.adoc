...
思考这个问题的最简单方法是从一个空白的 `MockRequest` 开始。你向其中添加的任何内容就是请求变成的样子。可能会让你感到惊讶的事情是，默认情况下没有上下文路径；没有 `jsessionid` cookie；没有转发、错误或异步分派；因此，没有实际的 JSP 渲染。相反，“`forwarded`”和“`redirected`”URL 保存在 `MockHttpResponseImpl` 中，并可以通过期望进行断言。
