...
xref:testing/annotations/integration-infra/annotation-contextcustomizerfactories.adoc[annotation support]
and the javadoc for
...
`META-INF/today.strategies` properties files. The `today.strategies` file for the
`infra-test` module can be viewed
{today-framework-code}/infra-test/src/main/resources/META-INF/today.strategies[here].
