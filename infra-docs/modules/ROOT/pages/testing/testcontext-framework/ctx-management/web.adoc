...
要让 TestContext 框架加载 `GenericWebApplicationContext` 而不是标准的 `ApplicationContext`，
...
请注意，Infra 对 `GenericWebApplicationContext` 实现的测试支持与对标准 `ApplicationContext`
实现的支持一致。使用 `GenericWebApplicationContext` 测试时，你可以通过
`@ContextConfiguration` 声明 XML 配置文件、Groovy 脚本或 `@Configuration` 类。
也可以使用其他测试注解，例如 `@ActiveProfiles`、`@TestExecutionListeners`、`@Sql`、
`@Rollback` 等。

本节剩余示例展示了加载 `GenericWebApplicationContext` 的多种配置选项。
