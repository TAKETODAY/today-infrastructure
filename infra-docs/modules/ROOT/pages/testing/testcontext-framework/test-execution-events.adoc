...
The `EventPublishingTestExecutionListener` introduced in TODAY Framework 5.2 offers an
...
Infra bean registered in the test `ApplicationContext`, and such beans may benefit
...
`infra.context.ApplicationListener` interface. Alternatively, listener
...
you can use Infra xref:integration/scheduling.adoc#scheduling-annotation-support-async[regular `@Async` support]
