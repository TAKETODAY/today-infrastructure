...
For example, the `infra-test` module declares all core default `TestExecutionListener`
implementations under the `infra.test.context.TestExecutionListener` key in
its {today-framework-code}/infra-test/src/main/resources/META-INF/today.strategies[`META-INF/today.strategies`
...
manner through their own `today.strategies` files.
