...
`infra-jdbc` module provides support for _initializing_ an embedded or existing database
...
`infra.test.context.jdbc` logging category to `DEBUG`.

If you want to see which SQL statements are being executed, set the
`infra.jdbc.datasource.init` logging category to `DEBUG`.
...
via `@SqlMergeMode(OVERRIDE)`. Consult the xref:testing/annotations/integration-infra/annotation-sqlmergemode.adoc[`@SqlMergeMode` annotation documentation section]
