...
`infra.cache.interceptor.KeyGenerator` interface.
...
`infra.cache.interceptor.CacheResolver` interface.
...
See xref:web/webmvc/mvc-core.adoc[the MVC section] for more information.
...
(`infra`) are not allowed in your code base. Similarly to the rest
