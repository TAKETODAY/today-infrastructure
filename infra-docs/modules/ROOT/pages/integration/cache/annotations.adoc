...
NOTE: `<cache:annotation-driven/>` 仅在定义它的同一个应用程序上下文中查找 `@Cacheable/@CachePut/@CacheEvict/@Caching`。这意味着，如果您将 `<cache:annotation-driven/>` 放在 `MockDispatcherHandler` 的 `WebApplicationContext` 中，它仅检查控制器中的 bean，而不检查服务中的 bean。有关更多信息，请参阅 xref:web/webmvc/mvc-core.adoc[MVC 部分]。
