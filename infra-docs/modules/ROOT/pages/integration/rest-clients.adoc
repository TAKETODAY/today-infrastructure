...
The TODAY Framework provides the following choices for making calls to REST endpoints:
...
The `infra-web` module contains the `HttpMessageConverter` interface for reading and writing the body of HTTP requests and responses through `InputStream` and `OutputStream`.
`HttpMessageConverter` instances are used on the client side (for example, in the `RestClient`) and on the server side (for example, in Web MVC REST controllers).
...
Refer to the {today-framework-api}/http/converter/HttpMessageConverter.html[`HttpMessageConverter` Javadoc] for the complete list.
...
| An `HttpMessageConverter` implementation that can read and write XML by using Infra `Marshaller` and `Unmarshaller` abstractions from the `infra.oxm` package.
...
* `ReactorClientHttpRequestFactory` for Reactor Netty's `HttpClient`

If no request factory is specified when the `RestClient` was built, it will use the Apache if they are available on the classpath.
...
It was introduced in 5.0 and offers an alternative to the `RestTemplate`, with support for synchronous, asynchronous, and streaming scenarios.
...
The `RestTemplate` provides a high-level API over HTTP client libraries in the form of a classic Infra Template class.
...
By default, it uses the `JdkClientHttpRequestFactory`, but this can be changed via the constructor.
...
The TODAY Framework lets you define an HTTP service as a Java interface with
`@HttpExchange` methods.
You can pass such an interface to `HttpServiceProxyFactory`
to create a proxy which performs requests through an HTTP client such as `RestClient`
or `WebClient`.
You can also implement the interface from an `@Controller` for server request handling.
...
a Infra `Part`, or Reactive Streams `Publisher` of any of the above.

| `Part`
| Add a request part from a `Part`, typically used in a Web MVC controller
...
Clients adapted to `RequestExecutionFactory` such as `RestClient`
support synchronous and asynchronous return values:
...
Clients adapted to `RequestExecutionFactory` such as `WebClient`, support all of above as well as reactive variants.
The table below shows Reactor types, but you can also use other reactive types that are supported through the `ReactiveAdapterRegistry`:
...
depends on how the underlying HTTP client is configured.
You can set a `blockTimeout`
value on the adapter level as well, but we recommend relying on timeout settings of the underlying HTTP client, which operates at a lower level and provides more control.
...
To customize this, register a response status handler that applies to all responses performed through the client:
...
To customize this, register an error handler that applies to all responses performed through the client:
