...
The `today-web` module contains the `HttpMessageConverter` interface for reading and writing the body of HTTP requests and responses through `InputStream` and `OutputStream`.
...
| An `HttpMessageConverter` implementation that can read and write XML by using Infra `Marshaller` and `Unmarshaller` abstractions from the `cn.taketoday.oxm` package.
...
| `MultipartFile`
| Add a request part from a `MultipartFile`, typically used in a Web MVC controller
...
Clients adapted to `ReactorHttpExchangeAdapter` such as `WebClient`, support all of above as well as reactive variants.
