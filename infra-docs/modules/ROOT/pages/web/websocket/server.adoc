...
`ExceptionWebSocketHandler` 捕获所有由任何 `WebSocketHandler` 方法引发的未捕获异常，并使用状态 `1011` 关闭 WebSocket 会话，这表示服务器错误。
...
WebSocket 和 SockJS 的默认行为是仅接受同源请求。
也可以允许所有或指定的一组来源。
此检查主要设计用于浏览器客户端。
没有任何阻止其他类型的客户端修改 `Origin` 标头值（有关详细信息，请参见
...
在此模式下，当启用 SockJS 时，IFrame 传输被禁用。
因此，当启用此模式时，不支持 IE6 到 IE9。
* 允许所有来源：要启用此模式，应将 `{asterisk}` 作为允许的来源值提供。
在此模式下，所有传输都可用。
