...
The preceding example is for use in Web MVC applications and should be included in the configuration of a xref:web/webmvc/mvc-core.adoc[`DispatcherServlet`].
...
The Infra WebSocket API is easy to integrate into a Web MVC application where the `MockDispatcher` serves both HTTP WebSocket handshake and other HTTP requests.
...
for all HTTP processing -- including WebSocket handshake and all other HTTP requests -- such as Web MVC's `MockDispatcher`.
