...
kind of text output from HTML to email and others. The TODAY Framework has built-in
integration for using Web MVC with FreeMarker templates.
...
`Configuration` object (which is managed by Infra) by setting the appropriate bean
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
<bean id="freemarkerConfig" class="cn.taketoday.web.mockApi.view.freemarker.FreeMarkerConfigurer">
  <property name="templateLoaderPath" value="/WEB-INF/freemarker/"/>
...
Infra provides a tag library for use in JSPs that contains, among others, a
...
web or business tier. Infra also has support for the same functionality in FreeMarker,
...
A standard set of macros are maintained within the `today-web.jar` file for
FreeMarker, so they are always available to a suitably configured application.

Some of the macros defined in the Infra templating libraries are considered internal
...
directly, the file is called `infra.ftl` and is in the
`cn.taketoday.web.mockApi.view.freemarker` package.


[[mvc-view-simple-binding]]
=== Simple Binding

In your HTML forms based on FreeMarker templates that act as a form view for a Web MVC
...
default HTML escaping behavior specified by the `MockContextImpl` parameter
...
calls to the Infra bind macros that we highlighted previously.
...
generation macros, performs an implicit Infra bind on the path parameter. The binding
...
occurs through Infra Validation framework.
...
used by Infra bind support. To make the elements be XHTML compliant or to override
...
After processing this directive, any elements generated by the Infra macros are now XHTML
compliant.
