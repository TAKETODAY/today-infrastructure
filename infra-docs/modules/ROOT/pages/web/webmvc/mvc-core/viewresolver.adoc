...
`RequestConext.forward` 执行分派。因此，您必须始终将 `InternalResourceViewResolver` 配置为视图解析器整体顺序中的最后一个。
...
{today-framework-api}/web/view/ContentNegotiatingViewResolver.html[`ContentNegotiatingViewResolver`]
