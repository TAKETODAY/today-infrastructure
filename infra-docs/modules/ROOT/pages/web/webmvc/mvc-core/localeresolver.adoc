...
Infra 架构的大部分都支持国际化，Infra Web MVC 框架也是如此。`DispatcherHandler` 让您可以使用客户端的区域设置自动解析消息。
这是通过 `LocaleResolver` 对象完成的。

当请求进来时，`DispatcherHandler` 会寻找一个区域设置解析器，如果找到，它会尝试使用它来设置区域设置。
通过使用 `HttpContext.getLocale()` 方法，您始终可以检索由区域设置解析器解析的区域设置。
...
如果可用，可以使用 `HttpContext.getTimeZone()` 方法获取用户的 `TimeZone`。
...
与 `CookieLocaleResolver` 相比，此策略将本地选择的区域设置存储在 Servlet 容器的 `Session` 中。
因此，这些设置对于每个会话都是临时的，因此在每个会话结束时会丢失。

请注意，与外部会话管理机制（如 Infra Session 项目）没有直接关系。
此 `SessionLocaleResolver` 针对当前 `MockRequest` 评估和修改相应的 `Session` 属性。
