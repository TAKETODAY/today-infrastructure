...
(see xref:web/webmvc/mvc-mockApi/container-config.adoc[Servlet Config]) all filters are automatically registered for all dispatch
...
`AbstractAnnotationConfigDispatcherServletInitializer` (see xref:web/webmvc/mvc-mockApi/container-config.adoc[Servlet Config])
