...
syntax of `{varName:regex}`. For example, given URL `"/today-web-4.0.jar"`, the following method
...
A response wrapper, applied in `jakarta.mockApi.http.HttpServlet`, ensures a `Content-Length`
