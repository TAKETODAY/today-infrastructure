...
dependencies {
  implementation 'cn.taketoday:infra-starter-web'   // <1>
  implementation 'cn.taketoday:infra-starter-netty' // <2>

}

----

<1> 上面的 `infra-starter-*` 和 Spring Boot 的 `Starters` 是一个概念，这样设计也是方便那些会用 Spring Boot 的开发者
...
添加了 `infra-starter-web` 依赖之后 框架会自动识别当前应用是一个 Web 应用。
会开启一个 Web 服务器 由于存在 `infra-starter-netty` 依赖，所以使用了 Netty 作为底层 Web 服务器支持。
