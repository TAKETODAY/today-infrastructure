 ...
 * with JUnit Jupiter by registering the {@link infra.test.context.junit.jupiter.InfraExtension} via a static field.
 ...
 * Platform, simply run {@link InfraJUnitJupiterTestSuite} as a JUnit 4 test.
 *
 * @author Sam Brannen
...
class RegisterExtensionInfraExtensionTests {
  ...
  void genericApplicationContextInjectedIntoMethod(GenericApplicationContext applicationContext) {
    assertThat(applicationContext).as("GenericApplicationContext should have been injected by Infra").isNotNull();
    assertThat(applicationContext.getBean("dilbert", Person.class)).isEqualTo(this.dilbert);
  }
  ...
  void autowiredParameterByTypeForSingleBean(@Autowired Dog dog) {
    assertThat(dog).as("Dogbert should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterByTypeForPrimaryBean(@Autowired Cat primaryCat) {
    assertThat(primaryCat).as("Primary cat should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterWithExplicitQualifier(@Qualifier("wally") Person person) {
    assertThat(person).as("Wally should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterWithImplicitQualifierBasedOnParameterName(@Autowired Person wally) {
    assertThat(wally).as("Wally should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterAsJavaUtilOptional(@Autowired Optional<Dog> dog) {
    assertThat(dog).as("Optional dog should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterThatDoesNotExistAsJavaUtilOptional(@Autowired Optional<Number> number) {
    assertThat(number).as("Optional number should have been @Autowired by Infra").isNotNull();
  ...
  void autowiredParameterThatDoesNotExistButIsNotRequired(@Autowired(required = false) Number number) {
    assertThat(number).as("Non-required number should have been @Autowired as 'null' by Infra").isNull();
  }

  @Test
  void autowiredParameterOfList(@Autowired List<Person> peopleParam) {
    assertThat(peopleParam).as("list of people should have been @Autowired by Infra").isNotNull();
  ...
  void valueParameterFromPropertyPlaceholder(@Value("${enigma}") Integer enigmaParam) {
    assertThat(enigmaParam).as("Enigma should have been injected via @Value by Infra").isNotNull();
  ...
  void valueParameterFromDefaultValueForPropertyPlaceholder(@Value("${bogus:false}") Boolean defaultValue) {
    assertThat(defaultValue).as("Default value should have been injected via @Value by Infra").isNotNull();
  ...
  void valueParameterFromSpelExpression(@Value("#{@dilbert.name}") String name) {
    assertThat(name).as(
            "Dilbert's name should have been injected via SpEL expression in @Value by Infra").isNotNull();
  ...
  void valueParameterFromSpelExpressionWithNestedPropertyPlaceholder(@Value("#{'Hello ' + ${enigma}}") String hello) {
    assertThat(hello).as("hello should have been injected via SpEL expression in @Value by Infra").isNotNull();
    assertThat(hello).as("hello from SpEL expression").isEqualTo("Hello 42");
  }

  @Test
  void junitAndInfraMethodInjectionCombined(@Autowired Cat kittyCat, TestInfo testInfo, ApplicationContext context,
    ...
    assertThat(context).as("ApplicationContext should have been injected by Infra").isNotNull();
    assertThat(kittyCat).as("Cat should have been @Autowired by Infra").isNotNull();
  }
