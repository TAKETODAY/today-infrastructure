...
class WebClientIntegrationTests {
  ...
  void retrieve(ClientHttpConnector connector) {
    ...
    expectRequest(request -> {
      Assertions.assertThat(request.getHeader(HttpHeaders.COOKIE)).isEqualTo("testkey=testvalue");
      Assertions.assertThat(request.getHeader("X-Test-Header")).isEqualTo("testvalue");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("*/*");
      Assertions.assertThat(request.getPath()).isEqualTo("/greeting");
    });
  }
  ...
  void retrieveJson(ClientHttpConnector connector) {
    ...
    expectRequestCount(1);
    expectRequest(request -> {
      Assertions.assertThat(request.getPath()).isEqualTo("/pojo");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("application/json");
    });
  }
  ...
  void retrieveJsonWithParameterizedTypeReference(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result)
            .assertNext(c -> Assertions.assertThat(c.getContainerValue()).isEqualTo(new Pojo("foofoo", "barbar")))
            .expectComplete().verify(Duration.ofSeconds(3));

    expectRequestCount(1);
    expectRequest(request -> {
      Assertions.assertThat(request.getPath()).isEqualTo("/json");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("application/json");
    });
  }
  ...
  void retrieveJsonArray(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result)
            .consumeNextWith(p -> Assertions.assertThat(p.getBar()).isEqualTo("bar1"))
            .consumeNextWith(p -> Assertions.assertThat(p.getBar()).isEqualTo("bar2"))
            .expectComplete()
            .verify(Duration.ofSeconds(3));

    expectRequestCount(1);
    expectRequest(request -> {
      Assertions.assertThat(request.getPath()).isEqualTo("/pojos");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("application/json");
    });
  }
  ...
  void retrieve500(ClientHttpConnector connector) {
    ...
    expectRequest(request -> {
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("*/*");
      Assertions.assertThat(request.getPath()).isEqualTo(path);
    });
  }
  ...
  void retrieve500AsEntity(ClientHttpConnector connector) {
    ...
    expectRequestCount(1);
    expectRequest(request -> {
      Assertions.assertThat(request.getPath()).isEqualTo("/");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("application/json");
    });
  }
  ...
  void retrieve555UnknownStatus(ClientHttpConnector connector) {
    ...
    expectRequest(request -> {
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("*/*");
      Assertions.assertThat(request.getPath()).isEqualTo("/unknownPage");
    });
  }
  ...
  void postPojoAsJson(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result)
            .consumeNextWith(p -> Assertions.assertThat(p.getBar()).isEqualTo("BARBAR"))
            .expectComplete()
            .verify(Duration.ofSeconds(3));

    expectRequestCount(1);
    expectRequest(request -> {
      Assertions.assertThat(request.getPath()).isEqualTo("/pojo/capitalize");
      Assertions.assertThat(request.getBody().readUtf8()).isEqualTo("{\"foo\":\"foofoo\",\"bar\":\"barbar\"}");
      Assertions.assertThat(request.getHeader(HttpHeaders.CONTENT_LENGTH)).isEqualTo("31");
      Assertions.assertThat(request.getHeader(HttpHeaders.ACCEPT)).isEqualTo("application/json");
      Assertions.assertThat(request.getHeader(HttpHeaders.CONTENT_TYPE)).isEqualTo("application/json");
    });
  }
  ...
  void exchangeForJsonAsResponseEntity(ClientHttpConnector connector) {
    ...
    StepVerifier.create(result)
            .consumeNextWith(entity -> {
              ...
              assertThat(entity.headers().getContentType()).isEqualTo(MediaType.APPLICATION_JSON);
              assertThat(entity.headers().getContentLength()).isEqualTo(31);
              Assertions.assertThat(entity.getBody()).isEqualTo(new Pojo("foofoo", "barbar"));
            })
            .expectComplete().verify(Duration.ofSeconds(3));
  ...
  void filter(ClientHttpConnector connector) {
    ...
    expectRequestCount(1);
    expectRequest(request -> Assertions.assertThat(request.getHeader("foo")).isEqualTo("bar"));
  }
  ...
  private void expectRequestCount(int count) {
    Assertions.assertThat(this.server.getRequestCount()).isEqualTo(count);
  }
