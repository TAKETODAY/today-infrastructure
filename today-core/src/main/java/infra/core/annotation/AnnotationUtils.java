...
public abstract class AnnotationUtils {
  ...
  public static <A extends Annotation> Set<A> getRepeatableAnnotations(
    ...
    RepeatableContainers repeatableContainers =
            containerAnnotationType != null
                    ? RepeatableContainers.explicit(annotationType, containerAnnotationType)
   ...
   * @param clazz the class to check for the annotation on (maybe {@code null})
   * @return the first {@link Class} in the inheritance hierarchy that
  ...
  public static boolean isAnnotationMetaPresent(Class<? extends Annotation> annotationType, @Nullable Class<? extends Annotation> metaAnnotationType) {
    ...
    // Exhaustive retrieval of merged annotations...
    return MergedAnnotations.from(annotationType, MergedAnnotations.SearchStrategy.INHERITED_ANNOTATIONS, RepeatableContainers.NONE)
    ...
    if (!methods.hasNestedAnnotation) {
      ...
      for (Method attribute : methods.attributes) {
        ...
        if (defaultValue != null) {
          result.put(attribute.getName(), new DefaultValueHolder(defaultValue));
        }
      }
    }
    else {
      // If we have nested annotations, we need them as nested maps
      var attributes = MergedAnnotation.valueOf(annotationType)
              .asMap(annotation -> new AnnotationAttributes(annotation.getType(), true), MergedAnnotation.Adapt.ANNOTATION_TO_MAP);
  ...
  public static boolean isSynthesizedAnnotation(@Nullable Annotation annotation) {
    try {
      ...
      // assume the annotation has not been synthesized by Infra.
      return false;
    }
  }
