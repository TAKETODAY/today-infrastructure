 ...
 * @since 4.0
 */
public class SettableFutureNotifier<V, F extends Future<V>> implements FutureListener<F> {
  ...
  /**
   * Link the {@link Future} and {@link SettableFuture} such
   * that if the {@link Future} completes the {@link SettableFuture}
   * will be notified. Cancellation is propagated both ways such
   * that if the {@link Future} is cancelled
   * the {@link SettableFuture} is cancelled and vise-versa.
   *
   * @param future the {@link Future} which will be
   ...
   */
  public static <V, F extends Future<V>> F cascade(final F future, final SettableFuture<? super V> settableFuture) {
    return cascade(true, future, settableFuture);
  }

  /**
   * Link the {@link Future} and {@link SettableFuture}
   * such that if the {@link Future} completes the
   * {@link SettableFuture} will be notified. Cancellation is propagated
   * both ways such that if the {@link Future} is cancelled
   * the {@link SettableFuture} is cancelled and vise-versa.
   *
