...
public class Indexer extends SpelNodeImpl {
  ...
  private ValueRef getValueRef(ExpressionState state, AccessMode accessMode) throws EvaluationException {
    ...
    if (target instanceof Map<?, ?> map) {
      Object key = index;
      TypeDescriptor mapKeyTypeDescriptor = targetDescriptor.getMapKeyDescriptor();
  ...
  private class ArrayIndexingValueRef implements ValueRef {
    ...
    public void setValue(@Nullable Object newValue) {
      TypeDescriptor elementType = this.typeDescriptor.getElementDescriptor();
  ...
  private class MapIndexingValueRef implements ValueRef {
    ...
    public TypedValue getValue() {
      ...
      return new TypedValue(value, this.mapEntryDescriptor.getMapValueDescriptor(value));
    }
  ...
  private class CollectionIndexingValueRef implements ValueRef {
    ...
    private void growCollectionIfNecessary() {
      if (this.index >= this.collection.size()) {
        ...
        TypeDescriptor elementType = this.collectionEntryDescriptor.getElementDescriptor();
        try {
  ...
  private class IndexAccessorValueRef implements ValueRef {
    ...
    public TypedValue getValue() {
      ...
      if (exception != null) {
        throw new SpelEvaluationException(getStartPosition(), exception,
                SpelMessage.EXCEPTION_DURING_INDEX_READ, this.index, this.typeDescriptor.toString());
      }
      throw new SpelEvaluationException(getStartPosition(),
    ...
    public void setValue(@Nullable Object newValue) {
      ...
      try {
        ...
        List<IndexAccessor> accessorsToTry = AstUtils.getAccessorsToTry(this.target, this.evaluationContext.getIndexAccessors());
        for (IndexAccessor indexAccessor : accessorsToTry) {
