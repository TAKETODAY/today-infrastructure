 ...
 * {@code infra.dockertest.max-parallel-tasks}. By default, only a
...
abstract class DockerTestBuildService implements BuildService<BuildServiceParameters.None> {
	...
	private static int maxParallelTasks(Project project) {
		Object property = project.findProperty("infra.dockertest.max-parallel-tasks");
		if (property == null) {
			return 1;
		}
