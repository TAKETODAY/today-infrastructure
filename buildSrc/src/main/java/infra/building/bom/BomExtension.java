...
public class BomExtension {
  ...
  private void addLibrary(Library library) {
    ...
    for (Library.Group group : library.getGroups()) {
      ...
      for (String bomImport : group.getBoms()) {
        ...
        dependencyHandler.add(BomPlugin.API_ENFORCED_CONFIGURATION_NAME, dependencyHandler.enforcedPlatform(bomDependency));
      }
    }
  }
  ...
  public static class LibraryHandler {
    ...
    private final ArrayList<Library.Group> groups = new ArrayList<>();

    private final ArrayList<Library.ProhibitedVersion> prohibitedVersions = new ArrayList<>();
    ...
    public void group(String id, Action<GroupHandler> action) {
      ...
      groups.add(new Library.Group(groupHandler.id, groupHandler.modules, groupHandler.plugins, groupHandler.imports));
    }

    public void prohibit(Action<ProhibitedHandler> action) {
      ProhibitedHandler handler = new ProhibitedHandler();
      action.execute(handler);
      this.prohibitedVersions.add(new Library.ProhibitedVersion(handler.versionRange, handler.startsWith,
