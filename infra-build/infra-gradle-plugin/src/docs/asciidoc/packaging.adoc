...
To do so, the embedded mockApi container dependencies should be added to the `providedRuntime` configuration, for example:
...
It is recommended that you only enable this option if you intend to execute it directly, rather than running it with `java -jar` or deploying it to a mockApi container.
...
First claiming `infra/app/loader/**` content for the `infra-app-loader` layer.
