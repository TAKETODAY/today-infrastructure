...
The plugin is https://plugins.gradle.org/plugin/cn.taketoday.application[published to Gradle's plugin portal] and can be applied using the `plugins` block:
...
The plugin is published to the Infra milestones repository.
...
The plugin is published to the Infra snapshots repository.
...
A typical Infra project will apply the {groovy-plugin}[`groovy`], {java-plugin}[`java`], or {kotlin-plugin}[`org.jetbrains.kotlin.jvm`] plugin as a minimum and also use the {dependency-management-plugin}[`io.spring.dependency-management`] plugin or Gradle's native bom support for dependency management.
For example:
...
To learn more about how the Infra plugin behaves when other plugins are applied please see the section on <<reacting-to-other-plugins, reacting to other plugins>>.
