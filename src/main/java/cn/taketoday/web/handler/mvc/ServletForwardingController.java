  ...
  protected ModelAndView handleRequestInternal(RequestContext request) throws Exception {
    ...
    if (useInclude(servletRequest, servletResponse)) {
      ...
      if (log.isTraceEnabled()) {
        log.trace("Included servlet [{}] in ServletForwardingController '{}'", servletName, beanName);
      }
    }
    else {
      rd.forward(servletRequest, servletResponse);
      if (log.isTraceEnabled()) {
        log.trace("Forwarded to servlet [{}] in ServletForwardingController '{}'", servletName, beanName);
      }
    }
