...
public class CacheableMappedHandlerRegistry extends MappedHandlerRegistry {
  ...
  protected Object lookupPatternHandler(String handlerKey) {
    ...
    if (handler == null) {
      handler = super.lookupPatternHandler(handlerKey);
      patternMatchingCache.put(handlerKey, handler == null ? EmptyObject.INSTANCE : handler);
    }
  ...
  protected ConcurrentMapCache createPatternMatchingCache() {
    return new ConcurrentMapCache("pattern-matching", 128);
  }
