 ...
 *
 * @author Josh Bloch
 * @author TODAY 2021/8/6 23:09
 * @since 4.0
 */
public abstract class AnnotationParser {
  static final MethodInvoker getAnnotationBytes = MethodInvoker.create(Method.class, "getAnnotationBytes");
   ...
   *         if an annotation is found to be
   *         malformed.
   */
   ...
   *
   * @param selectAnnotationClasses
   *         an array of annotation types to select when parsing
   */
  @SafeVarargs
  ...
  public static AnnotationAttributes[] readAnnotations(Executable executable) {
    Object rawAnnotations = getAnnotationBytes.invoke(executable, null);
    return readAnnotations((byte[]) rawAnnotations, executable.getDeclaringClass());
  }

  public static AnnotationAttributes[] readAnnotations(
          byte[] rawAnnotations, Class<?> container) {
    ConstantPool constantPool = SharedSecrets.getJavaLangAccess()
            .getConstantPool(container);
    return readAnnotations(rawAnnotations, constantPool, container);
  }
  ...
  public static AnnotationAttributes[] readAnnotations(
    ...
    AnnotationAttributes[] annotationAttributes = new AnnotationAttributes[numAnnotations];

    for (int i = 0; i < numAnnotations; i++) {
      annotationAttributes[i] = parseAnnotation3(buf, constPool, container);
    }
  ...
  private static Map<Class<? extends Annotation>, Annotation> parseAnnotations2(
    ...
    LinkedHashMap<Class<? extends Annotation>, Annotation> result = new LinkedHashMap<>();
    ByteBuffer buf = ByteBuffer.wrap(rawAnnotations);
    int numAnnotations = buf.getShort() & 0xFFFF;
    for (int i = 0; i < numAnnotations; i++) {
  ...
  private static Annotation[][] parseParameterAnnotations2(
    ...
    for (int i = 0; i < numParameters; i++) {
      int numAnnotations = buf.getShort() & 0xFFFF;
      ArrayList<Annotation> annotations = new ArrayList<>(numAnnotations);
      ...
      result[i] = annotations.toArray(Constant.EMPTY_ANNOTATION_ARRAY);
    }
    return result;
  }
   ...
   *
   * @param exceptionOnMissingAnnotationClass
   *         if true, throw
  ...
  private static AnnotationAttributes parseAnnotation3(
          ByteBuffer buf, ConstantPool constPool, Class<?> container) {

    int typeIndex = buf.getShort() & 0xFFFF;
    Class<? extends Annotation> annotationClass = getAnnotationClass(constPool, container, typeIndex);
    AnnotationType type = AnnotationType.getInstance(annotationClass);

    Map<String, Class<?>> memberTypes = type.memberTypes();
    AnnotationAttributes memberValues = new AnnotationAttributes(type.memberDefaults(), annotationClass);
    ...
    memberValues.trimToSize();
    return memberValues;
  }

  @SuppressWarnings("unchecked")
  private static Class<? extends Annotation> getAnnotationClass(
          ConstantPool constPool, Class<?> container, int typeIndex) {
  ...
  private static Annotation parseAnnotation2(ByteBuffer buf,
    ...
    String sig = "[unknown]";
    try {
      annotationClass = getAnnotationClass(constPool, container, typeIndex);
    }
   ...
   */
  @SuppressWarnings("unchecked")
  public static <T extends Annotation> T annotationForMap(
          final Class<T> type, final Map<String, Object> memberValues) {
   ...
   * u2   const_value_index;
   */
  private static Object parseConst(
          int tag, ByteBuffer buf, ConstantPool constPool) {
    int constIndex = buf.getShort() & 0xFFFF;
    switch (tag) {
      case 'B':
        return (byte) constPool.getIntAt(constIndex);
      case 'C':
        return (char) constPool.getIntAt(constIndex);
      case 'D':
        return constPool.getDoubleAt(constIndex);
      case 'F':
        return constPool.getFloatAt(constIndex);
      case 'I':
   ...
   *
   * @param complete
   *         true if the byte buffer points to the beginning
