...
import infra.beans.testfixture.beans.TestAnnotation;
...
class BeanFactoryUtilsTests {
  ...
  private void testFindAnnotationOnBean(BeanFactory lbf) {
    assertControllerAdvice(lbf, "controllerAdvice");
    assertControllerAdvice(lbf, "restControllerAdvice");
  }

  private void assertControllerAdvice(BeanFactory lbf, String beanName) {
  ...
  void testIsSingletonAndIsPrototype(BeanFactory lbf) {
  ...
  void testSupportsMultipleTypes(BeanFactory lbf) {
