 ...
 */
public class FindByIdQuery extends SimpleSelectQueryStatement implements QueryStatement, DebugDescriptive {
  private final Object id;

  public FindByIdQuery(Object id) {
    this.id = id;
  }

  @Override
  protected void renderInternal(EntityMetadata metadata, SimpleSelect select) {
    select.addRestriction(Restrictions.equal(metadata.idProperty().getColumnName()));
    select.limit(1);
  }
