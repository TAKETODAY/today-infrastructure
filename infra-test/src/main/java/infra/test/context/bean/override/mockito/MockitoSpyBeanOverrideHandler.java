...
class MockitoSpyBeanOverrideHandler extends AbstractMockitoBeanOverrideHandler {
  ...
  private Object createSpy(String name, Object instance) {
    InfraMockResolver.rejectUnsupportedSpyTarget(name, instance);
   ...
   * Infra AOP when the verification of a spy starts.
   */
  private static final class InfraAopBypassingVerificationStartedListener implements VerificationStartedListener {

    @Override
    public void onVerificationStarted(VerificationStartedEvent event) {
      event.setMock(InfraMockResolver.getUltimateTargetObject(event.getMock()));
    }
  }
