...
class BeanOverrideBeanFactoryPostProcessor implements BeanFactoryPostProcessor, Ordered {
  ...
  private static final Logger logger = LoggerFactory.getLogger(BeanOverrideBeanFactoryPostProcessor.class);
  ...
  private void registerBeanOverride(ConfigurableBeanFactory beanFactory, BeanOverrideHandler handler,
  ...
  private void replaceOrCreateBean(ConfigurableBeanFactory beanFactory, BeanOverrideHandler handler,
   ...
   */
  private void wrapBean(ConfigurableBeanFactory beanFactory, BeanOverrideHandler handler) {
    String beanName = handler.getBeanName();
    Field field = handler.getField();
  ...
  private static @Nullable String getBeanNameForType(ConfigurableBeanFactory beanFactory, BeanOverrideHandler handler,
  ...
  private static Set<String> getExistingBeanNamesByType(ConfigurableBeanFactory beanFactory,
   ...
   * @see infra.beans.factory.support.StandardBeanFactory#determineAutowireCandidate
   * @since 5.0
   */
  private static @Nullable String determineUniqueCandidate(ConfigurableBeanFactory beanFactory,
   ...
   * @see infra.beans.factory.support.StandardBeanFactory#determinePrimaryCandidate
   */
  private static @Nullable String determinePrimaryCandidate(ConfigurableBeanFactory beanFactory,
  ...
  private static void destroySingleton(ConfigurableBeanFactory beanFactory, String beanName) {
    if (!(beanFactory instanceof StandardBeanFactory dlbf)) {
      throw new IllegalStateException("Cannot process bean override with a BeanFactory " +
              "that does not implement StandardBeanFactory: " + beanFactory.getClass().getName());
    }
