...
package infra.messaging.handler;
...
public class DestinationPatternsMessageCondition
  ...
  private static Set<String> prependLeadingSlash(String[] patterns, RouteMatcher routeMatcher) {
    ...
    for (String pattern : patterns) {
      if (slashSeparator && StringUtils.isNotEmpty(pattern) && !pattern.startsWith("/")) {
        pattern = "/" + pattern;
      }
      result.add(pattern);
    }
   ...
   * the patterns in "other" using {@link infra.util.PathMatcher#combine(String, String)}.
   ...
   * {@link infra.util.PathMatcher#getPatternComparator(String)}.
   *
