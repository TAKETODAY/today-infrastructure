...
class InternalPathPatternParser {
  ...
  private boolean isDoubleWildcard(char separator) {
    ...
    return ((this.pos + 2) >= this.pathPatternLength)
  ...
  private void pushPathElement(PathElement newPathElement) {
    if (newPathElement instanceof CaptureSegmentsPathElement
      ...
      if (this.currentPE == null) {
        ...
        throw new IllegalStateException("Expected SeparatorPathElement before %s but was %s"
                .formatted(newPathElement.getClass().getName(), this.currentPE));
      }
    }
    else {
      if (this.headPE == null) {
    ...
    if (this.variableCaptureCount > 0) {
      if (this.variableCaptureCount == 1 && this.pathElementStart == this.variableCaptureStart
        ...
        if (this.isCaptureSegmentsVariable) {
          ...
          try {
            newPE = new CaptureVariablePathElement(
                    this.pathElementStart, getPathElementText(), this.parser.isCaseSensitive(), separator);
          }
          catch (PatternSyntaxException pse) {
            throw new PatternParseException(
                    pse, findRegexStart(this.pathPatternData, this.pathElementStart) + pse.getIndex(),
        ...
        if (this.isCaptureSegmentsVariable) {
          throw new PatternParseException(this.pathElementStart,
                  this.pathPatternData, PatternMessage.CAPTURE_ALL_IS_STANDALONE_CONSTRUCT);
        }
        RegexPathElement newRegexSection = new RegexPathElement(
