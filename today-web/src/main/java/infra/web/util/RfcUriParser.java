...
abstract class RfcUriParser {
  ...
  private static final Logger logger = LogDelegateFactory.getHiddenLog(RfcUriParser.class);
  ...
  private static class InternalParser {
    ...
    public void advanceTo(State state) {
      if (logger.isTraceEnabled()) {
        logger.trace("{} -> {}, index={}, componentIndex={}", this.state, state, this.index, this.componentIndex);
      }
      this.state = state;
    ...
    private String captureComponent(String logPrefix) {
      ...
      if (logger.isTraceEnabled()) {
        logger.trace("{} set to '{}'", logPrefix, value);
      }
      return value;
    }
    ...
    public String toString() {
      return "[State=%s, index=%d, componentIndex=%d, uri='%s', scheme='%s', user='%s', host='%s', path='%s', port='%s', query='%s', fragment='%s']"
              .formatted(this.state, this.index, this.componentIndex, this.uri, this.scheme,
                      this.user, this.host, this.path, this.port, this.query, this.fragment);
    }
  }
