...
class ServerSentEventHttpMessageReaderTests extends AbstractLeakCheckingTests {
  ...
  void canRead() {
    ...
    assertThat(reader.canRead(ResolvableType.forClass(infra.http.ServerSentEvent.class), new MediaType("foo", "bar"))).isTrue();
  }
  ...
  void readServerSentEvents() {
    ...
    StepVerifier.create(events)
            .expectNext(infra.http.ServerSentEvent.builder().id("c42").event("foo")
                    .retry(Duration.ofMillis(123)).comment("bla\nbla bla\nbla bla bla").data("bar").build())
            .expectNext(infra.http.ServerSentEvent.builder().id("c43").event("bar")
  ...
  void readServerSentEventsWithMultipleChunks() {
    ...
    Flux<infra.http.ServerSentEvent> events = reader
            .read(ResolvableType.forClassWithGenerics(infra.http.ServerSentEvent.class, String.class),
                    request, Collections.emptyMap()).cast(infra.http.ServerSentEvent.class);
