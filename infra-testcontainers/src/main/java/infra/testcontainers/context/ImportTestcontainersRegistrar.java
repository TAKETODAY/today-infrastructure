...
class ImportTestcontainersRegistrar implements ImportBeanDefinitionRegistrar {
  ...
  private static final String DYNAMIC_PROPERTY_SOURCE_CLASS = "infra.test.context.DynamicPropertySource";
  ...
  ImportTestcontainersRegistrar(Environment environment) {
    ...
    this.dynamicPropertySourceMethodsImporter = (!ClassUtils.isPresent(DYNAMIC_PROPERTY_SOURCE_CLASS)) ? null
            : new DynamicPropertySourceMethodsImporter();
  }

  @Override
  public void registerBeanDefinitions(AnnotationMetadata importMetadata, BootstrapContext context) {
    MergedAnnotation<ImportTestcontainers> annotation = importMetadata.getAnnotations()
    ...
    if (ObjectUtils.isEmpty(definitionClasses)) {
      Class<?> importingClass = ClassUtils.resolveClassName(importMetadata.getClassName(), null);
      definitionClasses = new Class<?>[] { importingClass };
    }
    registerBeanDefinitions(context.getRegistry(), definitionClasses);
  }
