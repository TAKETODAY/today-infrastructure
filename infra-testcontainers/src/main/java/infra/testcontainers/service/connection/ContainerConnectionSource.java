...
public final class ContainerConnectionSource<C extends Container<?>> implements OriginProvider {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ContainerConnectionSource.class);
  ...
  public boolean accepts(@Nullable String requiredConnectionName, Class<?> requiredContainerType,
    ...
    if (StringUtils.hasText(requiredConnectionName)
      ...
      logger.trace(LogMessage.from(() -> "%s not accepted as source connection name '%s' does not match required connection name '%s'"
              .formatted(this, this.connectionName, requiredConnectionName)));
      return false;
    }
    if (!requiredContainerType.isAssignableFrom(this.containerType)) {
      logger.trace(LogMessage.from(() -> "%s not accepted as source container type %s is not assignable from %s"
    ...
    if (!this.connectionDetailsTypes.isEmpty() && this.connectionDetailsTypes.stream()
      ...
      logger.trace(LogMessage.from(() -> "%s not accepted as source connection details types %s has no element assignable from %s"
              .formatted(this, this.connectionDetailsTypes.stream().map(Class::getName).toList(),
                      requiredConnectionDetailsType.getName())));
      return false;
    }
    logger.trace(LogMessage.from(() -> "%s accepted for connection name '%s' container type %s, connection details type %s"
