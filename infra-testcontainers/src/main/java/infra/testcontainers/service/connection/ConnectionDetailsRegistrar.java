...
class ConnectionDetailsRegistrar {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ConnectionDetailsRegistrar.class);

  private final BeanFactory beanFactory;

  private final ConnectionDetailsFactories connectionDetailsFactories;

  ConnectionDetailsRegistrar(BeanFactory beanFactory, ConnectionDetailsFactories connectionDetailsFactories) {
  ...
  private <T> void registerBeanDefinition(BeanDefinitionRegistry registry, ContainerConnectionSource<?> source,
    ...
    if (!ObjectUtils.isEmpty(existingBeans)) {
      logger.debug(LogMessage.from(() -> "Skipping registration of %s due to existing beans %s".formatted(source,
    ...
    logger.debug(LogMessage.from(() -> "Registering '%s' for %s".formatted(beanName, source)));
