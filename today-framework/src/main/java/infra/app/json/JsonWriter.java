...
public interface JsonWriter<T> {
  ...
  final class Members<T> {
    ...
    Members(Consumer<Members<T>> members, boolean contributesToExistingSeries) {
      Assert.notNull(members, "'members' is required");
      members.accept(this);
    ...
    public <V> Member<V> add(String name, Function<T, V> extractor) {
      ...
      Assert.notNull(extractor, "'extractor' is required");
    ...
    public void applyingNameProcessor(NameProcessor nameProcessor) {
      Assert.notNull(nameProcessor, "'nameProcessor' is required");
    ...
    public void applyingValueProcessor(ValueProcessor<?> valueProcessor) {
      Assert.notNull(valueProcessor, "'valueProcessor' is required");
      this.jsonProcessors.valueProcessors().add(valueProcessor);
    }

    private <V> Member<V> addMember(@Nullable String name, Function<T, V> extractor) {
  ...
  final class Member<T> {
    ...
    Member(int index, @Nullable String name, Extractor<T> extractor) {
      this.index = index;
      this.name = name;
    ...
    public Member<T> whenHasLength() {
      return when((instance) -> instance != null && StringUtils.isNotEmpty(instance.toString()));
    }
    ...
    public <E, N, V> Member<T> usingExtractedPairs(BiConsumer<T, Consumer<E>> elements,
      ...
      Assert.notNull(valueExtractor, "'valueExtractor' is required");
    ...
    public <N, V> Member<T> usingPairs(BiConsumer<T, BiConsumer<N, V>> pairs) {
      Assert.notNull(pairs, "'pairs' is required");
  ...
  record MemberPath(MemberPath parent, String name, int index) {
    ...
    public MemberPath child(String name) {
      return (StringUtils.isEmpty(name)) ? this : new MemberPath(this, name, UNINDEXED);
    }
