...
class ClientHttpConnectorFactoryConfigurationTests {
  ...
  void shouldApplyHttpClientMapper() {
    ...
    new ReactiveWebApplicationContextRunner()
            .withConfiguration(AutoConfigurations.of(infra.app.config.web.reactive.client.ClientHttpConnectorFactoryConfiguration.ReactorNetty.class))
            .withUserConfiguration(CustomHttpClientMapper.class)
            .run((context) -> {
              context.getBean(infra.app.config.web.reactive.client.ReactorClientHttpConnectorFactory.class).createClientHttpConnector(sslBundle);
  ...
  void shouldNotConfigureReactiveHttpClient5WhenHttpCore5ReactiveJarIsMissing() {
    new ReactiveWebApplicationContextRunner()
            ...
            .withConfiguration(AutoConfigurations.of(infra.app.config.web.reactive.client.ClientHttpConnectorFactoryConfiguration.HttpClient5.class))
  ...
  static class CustomHttpClientMapper {
    ...
    @Bean
    infra.app.config.web.reactive.client.ReactorNettyHttpClientMapper clientMapper() {
      return (client) -> {
        called = true;
